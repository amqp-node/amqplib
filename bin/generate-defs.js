...
println(
  'module.exports.encodeProperties =',
...
function checkAssignArg(a) {
  ...
  if (a.default !== undefined) {
    ...
    println(
      'throw new Error("Missing value for mandatory field \'%s\'");',
      a.name
    )
  }
  ...
  println(
    '"Field \'%s\' is the wrong type; must be %s");',
    a.name,
    typeDesc(a.type)
  )
  println('}') // type test
...
function assignTable(a) {
  ...
  println(
    'var %s = SCRATCH.slice(scratchOffset, scratchOffset + len);',
    varname
  )
...
function encoderFn(method) {
  ...
  for (let i = 0, len = args.length; i < len; i++) {
    ...
    switch (a.type) {
      ...
      case 'timestamp':
        checkAssignArg(a)
        println('buffer.writeBigUInt64BE(val, offset); offset += 8;')
        break
      case 'bit':
        checkAssignArg(a)
      ...
      case 'shortstr':
        ...
        println(
          'buffer.write(val, offset, "utf8"); offset += %s;',
          stringLenVar(a)
        )
        break
      case 'longstr':
  ...
  for (let i = 0, num = args.length; i < num; i++) {
    ...
    if (p.type === 'bit') {
      ...
      switch (p.type) {
        ...
        case 'longlong':
        case 'timestamp':
          println('buffer.writeBigUInt64BE(val, offset);')
          println('offset += 8;')
          break
        case 'shortstr':
