  ...
  */
  open(allFields, openCallback0) {
    ...
    function afterStartOk(reply) {
      switch (reply.id) {
        ...
        case defs.ConnectionClose:
          bail(
            new Error(
              fmt('Handshake terminated by server: %s', closeMsg(reply))
            )
          )
          break
        case defs.ConnectionTune:
          ...
          tunedOptions.channelMax = negotiate(
            fields.channelMax,
            allFields.channelMax
          )
          tunedOptions.heartbeat = negotiate(
            fields.heartbeat,
            allFields.heartbeat
          )
          try {
        ...
        default:
          bail(
            new Error(
              fmt(
                'Expected connection.secure, connection.close, ' +
    ...
    function endWhileOpening(err) {
      bail(
        err || new Error('Socket closed abruptly ' + 'during opening handshake')
      )
    }
    ...
    function succeed(ok) {
      ...
      self.stream.on(
        'end',
        self.onSocketError.bind(self, new Error('Unexpected close'))
      )
  ...
  sendContent(channel, body) {
    ...
    for (var offset = 0; offset < body.length; offset += maxBody) {
      ...
      var slice =
        end > body.length ? body.subarray(offset) : body.subarray(offset, end)
