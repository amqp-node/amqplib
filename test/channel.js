...
function channel_handshake(send, wait) {
  return wait(defs.ChannelOpen)().then(function (open) {
    ...
    resolve(
      (function () {
        ...
        return new Promise(function (resolve, reject) {
          ch._rpc(defs.ChannelOpen, { outOfBand: "" }, defs.ChannelOpenOk, function (err, result) {
            if (err) {
              reject(err);
            } else {
              resolve(result);
  ...
  test(
    ...
    channelTest(
      ...
      function (send, wait, done, ch) {
        ...
        return wait(defs.BasicQos)().then(sendOk).then(wait(defs.BasicQos)).then(sendOk).then(wait(defs.BasicQos)).then(sendOk).then(succeed(done), fail(done));
      },
    ),
  );
  ...
  test(
    ...
    channelTest(
      function (ch, done) {
        ...
        open(ch).then(function () {
          ch._rpc(defs.BasicRecover, { requeue: true }, defs.BasicRecoverOk, function (err) {
