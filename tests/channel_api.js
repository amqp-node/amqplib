...
suite('assert, check, delete', function () {
  ...
  chtest('assert and check exchange', function (ch) {
    return ch
      .assertExchange('test.check-exchange', 'direct', EX_OPTS)
      .then(function (eok) {
  ...
  chtest('fail on reasserting queue with different options', function (ch) {
    ...
    return ch
      ...
      .then(function () {
        return expectFail(
          ch.assertQueue(q, { durable: false, autoDelete: false })
        )
      })
  })
...
suite('binding, consuming', function () {
  ...
  chtest('unbind queue', function (ch) {
    ...
    return Promise.all([
      ...
      .then(function () {
        // message got through!
        return ch.get(q, { noAck: true }).then(function (m) {
          assert(m)
        })
      })
      .then(function () {
  ...
  chtest('cancel consumer', function (ch) {
    ...
    var recv1 = new Promise(function (resolve, reject) {
      Promise.all([
        ...
        ch.consume(q, resolve, { noAck: true }).then(function (ok) {
          ctag = ok.consumerTag
