...
const connectionString = process.env.URL || 'amqp://localhost'
...
suite('bindings', function () {
  ...
  channel_test('bind exchange', function (ch, done) {
    ch.assertExchange(
      ...
      'fanout',
      {},
      kCallback(function (ex1) {
        ch.assertExchange(
          'test.cb.bindex2',
          'fanout',
          {},
          kCallback(function (ex2) {
            ch.bindExchange(
              ex1.exchange,
              ex2.exchange,
              '',
      ...
      waitForMessages(ch, q.queue, function (e, _) {
        ...
        ch.get(q.queue, { noAck: true }, function (e, m) {
          if (e != null) return done(e)
            ...
            return done(
              new Error(
                'Messages do not match: ' + msg + ' =/= ' + m.content.toString()
              )
            )
        })
      })
