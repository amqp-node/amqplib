...
// `sendToQueue` both return a promise that will be resolved (on
// 'ack') or rejected (on 'nack').

function ConfirmChannel(connection) {
  Channel.call(this, connection);
  this.lwm = 1; // the least, unconfirmed deliveryTag
  this.unconfirmed = []; // rolling window of delivery promises
  this.on('ack', this._confirm.bind(this, true));
  this.on('nack', this._confirm.bind(this, false));
}
...
CC.publish = function(exchange, routingKey, content, options) {
  var confirm = defer();
  this.unconfirmed.push(confirm);
  C.publish.call(this, exchange, routingKey, content, options);
  return confirm.promise;
}

CC._confirm = function(success, f) {
  var tag = f.fields.deliveryTag;
  var multi = f.fields.multiple;

  var confirm = (success)
    ? function(c) { if (c) c.resolve(); }
    : function(c) { if (c) c.reject(); }
