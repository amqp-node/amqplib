...
function normaliseRecoveryOptions(recovery) {
  ...
  const maxRetries = source.maxRetries === undefined ? source.maxAttempts : source.maxRetries;
  const parsedMaxRetries = maxRetries == null ? Infinity : toFiniteNumber(maxRetries, Infinity);
...
function runSetup(setup, model, client) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      if (setup.length >= 3) {
        setup(model, client, done);
      } else {
        Promise.resolve(setup(model, client)).then(() => done(), done);
      }
    } catch (err) {
      done(err);
    }
...
class RecoveringCore extends EventEmitter {
  ...
  _bindModel(model) {
    ...
    const onConnectionError = (err) => this.emit('connectionError', err);
  ...
  _unbindModel() {
    ...
    const { model, onClose, onBlocked, onUnblocked, onConnectionError } = this._modelListeners;
  ...
  async _connect() {
    ...
    try {
      ...
      await runSetup(this._recovery.setup, model, this);
      if (this._stopped) {
        await this._closeModelNoThrow(model);
        return;
      }
  ...
  _scheduleReconnect(err) {
    ...
    this.emit('reconnectScheduled', { attempt, delay, error: err });
...
function wireCommonEvents(source, target) {
  ['connect', 'disconnect', 'connectFailed', 'reconnectScheduled', 'reconnectFailed', 'blocked', 'unblocked', 'connectionError'].forEach(
    (eventName) => {
