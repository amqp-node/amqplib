...
C.maybeReconnect = function(err, stack_capture_message) {
  ...
  var reconnect = this.reconnect && !isChannelError(err);
  if (!reconnect){
    this.emit('error', err);
  }
  ...
  var closed = this.toClosed(s, err);
  if (reconnect){
    return this.tryReconnect(err, closed, this.reconnectRetries);
  } else {
...
C.tryReconnect = function(err, closed, retries) {
  if(this.reconnectTimeoutRef !== undefined){
  ...
  connect.reconnect(this, function(reconnectError, conn) {
    if(reconnectError === null) {
      ...
      return self.recover(function() {});
    } else {
      if(Number.isInteger(retries)
         && Number.isInteger(self.reconnectTimeout)
         && retries > 0) {
        self.reconnectTimeoutRef = setTimeout(
          function(){
            self.tryReconnect(reconnectError, closed, retries - 1);
          },
  ...
  function recoverNext(self, error, n) {
    if(error !== null){
      ...
      if(ch !== null && ch.channel){
        ch.channel.recoverChannel(function(error){
...
C.startHeartbeater = function() {
  if (this.heartbeat === 0) return null;
    ...
    hb.on('timeout', function() {
      ...
      // TODO: reconnect on missed heartbeat?
