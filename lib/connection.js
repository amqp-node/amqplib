...
C.onSocketError = function(err) {
  if (!this.expectSocketClose) {
    ...
    //TODO: not emit error when reconnect?
    ...
    if (this.reconnect_enabled){
      return this.tryReconnect(s, err, closed);
    } else {
      return closed;
    }
  }
...
C.tryReconnect = function(s, err, closed){
  var self = this;
  revalidateSend(this);
  connect.reconnect(this, function(err, conn){
    if(err === null){
      return;
    } else {
      self.emit('error', err);
      return closed;
      // self.tryReconnect(s, err);
    }
  });
};

C.reconnect = function(sock){
