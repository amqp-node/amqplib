...
function invalidateSend(conn, msg, stack) {
  conn.sendMethod = conn.sendContent = conn.sendMethodAndProperties =
    invalidOp(msg, stack);
}
...
C.toClosing = function(cb) {
  ...
  invalidateSend(this, 'Connection closing', stack);
};

C._closeChannels = function() {
...
C.toClosed = function() {
  ...
  var stack = stackCapture();
  invalidateSend(this, 'Connection closed', stack);
  this.accept = invalidOp('Connection closed', stack);
