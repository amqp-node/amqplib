...
function channel0(connection) {
  return function(f) {
    ...
    if (f === HEARTBEAT); // ignore; it's already counted as activity
      ...
      if (e.code === defs.constants.CONNECTION_FORCED &&
          connection.recover &&
          connection.recoverOnServerClose) {
        connection.maybeReconnect(e, emsg);
      } else {
        if (isFatalError(e)) {
...
C.onSocketError = function(err) {
  if (!this.expectSocketClose) {
    ...
    this.maybeReconnect(err, 'Socket error');
  }
};

C.maybeReconnect = function(err, stack_capture_message) {
  // Reconnection is configured and error is recoverable (not a channel error)
  var needRecover = this.recover && !isChannelError(err);
  if (!needRecover){
    this.emit('error', err);
  }
  var s = stackCapture(stack_capture_message);
  // Need to capture old channels before closing
  this.old_channels = this.channels.slice();
  if (needRecover){
    this.toFreezed(s, err);
    var random_wait = Math.floor(Math.round() * this.recoverAfterRandom);
    this.scheduleReconnectAfter(this.recoverAfter + random_wait, err, this.recoverRetries);
  } else {
    this.toClosed(s, err);
    delete this.old_channels;
  }
};

C.scheduleReconnectAfter = function(after, error, retries) {
  var self = this;
  if(after && after > 0) {
    this.recoverTimeoutRef = setTimeout(
      function(){
        self.tryReconnect(error, retries);
      },
      after);
  } else {
    self.tryReconnect(error, retries);
...
C.tryReconnect = function(err, retries) {
  if(this.recoverTimeoutRef !== undefined){
    clearTimeout(this.recoverTimeoutRef);
  }
  var self = this;
  revalidateSend(this);
  connect.reconnect(this, function(reconnectError, conn) {
    if(reconnectError === null) {
      // Recover channels
      // Not sure what the error callback should be
      return self.doRecover(function() {});
    } else {
      if((typeof retries === 'number')
         && (typeof self.recoverTimeout === 'number')
         && retries > 0) {
        self.scheduleReconnectAfter(self.recoverTimeout,
                                    reconnectError,
                                    retries - 1);
      } else {
        self.emit('error', reconnectError);
      }
    }
  });
};

C.changeSocket = function(sock){
  // Reassign socket to existing connection
  // TODO: need this?
  EventEmitter.call( this );
  var stream = this.stream = wrapStream(sock);
  this.muxer = new Mux(stream);
  ...
  this.expectSocketClose = false;
  this.freeChannels = new BitSet();
  this.channels[0].buffer = sock;

  return this;
};

C.doRecover = function(callback) {
  // Recover channels
  function recoverNext(self, error, n) {
    if(error !== null){
      callback(error);
    } else if(self.old_channels == undefined){
      // Assume we're done
      callback(null, self);
    } else if(n == self.old_channels.length){
      delete self.old_channels;
      callback(null, self);
    } else {
      var ch = self.old_channels[n];
      if(ch !== null && ch.channel){
        ch.channel.recoverChannel(
          self.recoverTopology,
          function(error){
            recoverNext(self, error, n+1);
          });
      } else {
        recoverNext(self, error, n+1);
      }
...
function invalidateSend(conn, msg, stack, freeze) {
  if(freeze === true) {
...
// Cleanup invalidOp callbacks
function revalidateSend(conn) {
  delete conn.sendMethod;
  delete conn.sendContent;
  delete conn.sendMessage;
  delete conn.close;
}
...
C._closeChannels = function(capturedStack, freeze) {
  for (var i = 1; i < this.channels.length; i++) {
    var ch = this.channels[i];
    if (ch !== null) {
      ch.channel.toClosed(capturedStack, freeze); // %%% or with an error? not clear
    }
  }
};

C.toFreezed = function(capturedStack, maybeErr) {
  this.toClosed(capturedStack, maybeErr, true);
}

// A close has been confirmed. Cease all communication.
C.toClosed = function(capturedStack, maybeErr, freeze) {
  this._closeChannels(capturedStack, freeze);
  var info = fmt('Connection closed (%s)',
                 (maybeErr) ? maybeErr.toString() : 'by client');
  // Tidy up, invalidate enverything, dynamite the bridges.
  invalidateSend(this, info, capturedStack, freeze);
...
C.startHeartbeater = function() {
  if (this.heartbeat === 0) return null;
    ...
    hb.on('timeout', function() {
      var errmsg = 'Heartbeat timeout';
      var hberr = new Error(errmsg);
      if(self.recover && self.recoverOnMissedHeartbeat){
        self.maybeReconnect(hberr, errmsg);
      } else {
        self.emit('error', hberr);
        var s = stackCapture(errmsg);
