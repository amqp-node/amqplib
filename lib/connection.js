...
Connection.mainAccept = function(frame) {
  ...
  else this.closeBecause("Frame on unknown channel: " + frame,
                         defs.constants.CHANNEL_ERROR);
};

// Handle anything that comes through on channel 0, that's the
// connection control channel. This is only used once
// Connection.mainAccept is installed as the frame handler, after the
// opening handshake.
function channel0(connection) {
  return function(f) {
    ...
    if (f.id === defs.ConnectionClose) {
      ...
      var err = new Error('Connection closed: ' + f.fields.replyText);
      connection.toClosed(err);
    }
    else {
      connection.closeBecause("Unexpected frame on channel 0: " + f,
...
C.toClosed = function(err) {
  ...
  this.sendMethod = this.sendContent = Connection.closedSend;
  this.accept = Connection.closedAccept;
  this.stream.end();
