...
C.onSocketError = function(err) {
  if (!this.expectSocketClose) {
    ...
    // Reconnect if configured and the socket is dead.
    // This is to avoid reconnection on channel errors.
    var reconnect = this.reconnect && this.isSocketAlive()
    if (!reconnect){
      this.emit('error', err);
    }
...
C.isSocketAlive = function() {
  // Assume socket is alive if stream is readable
  // TODO: a better way to check?
