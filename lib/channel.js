...
C._rpc = function(method, fields, expect) {
  ...
  return reply.promise.then(function(f) {
    if (f.id === expect)
      ...
      var err = "Expected " + expect + " but got " + JSON.stringify(f);
      self.closeBecause(err, defs.constants.CHANNEL_ERROR);
      throw new Error(err); // to reject the implicit promise returned
    }
  }, function(close) {
    ...
    var errmsg = (method === closeReason)
      ? "Operation failed: " + method + "; " + closeMsg(close)
      : "Channel closed by server: " + closeMsg(close);
...
C.closeBecause = function(reason, code) {
  ...
  // If we're closing for any reason other than the application wanted
  // to, it's an error
  var err;
...
C.acceptMessageFrame = function(f) {
  try {
    ...
    this.closeBecause(msg, defs.constants.UNEXPECTED_FRAME);
  }
};
...
function acceptDeliveryOrReturn(f) {
  ...
  else throw "Unexpected BasicDeliver or BasicReturn, not " + f;
  ...
  function content(f) {
    if (f.content) {
      ...
      if (remaining === 0) {
        ...
        throw "Too much content sent! Expected " + totalSize + " bytes";
      }
      else {
        if (buffers !== null)
          buffers.push(f.content);
...
C.accept = function(f) {
  ...
  case defs.BasicCancel:
    ...
    return this.closeBecause("Cancel not implemented",
                             defs.constants.NOT_IMPLEMENTED);
  case defs.BasicFlow:
    // RabbitMQ doesn't send this, it just blocks the TCP socket
    return this.closeBecause("Flow not implemented",
                             defs.constants.NOT_IMPLEMENTED);
