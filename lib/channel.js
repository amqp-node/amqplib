...
function Channel(connection) {
  ...
  this.on('ack', this.handleConfirm.bind(this, function(cb) {
    if (cb) {
      var err = null;
      if (self._returnMessage && self._returnMessage.fields.replyCode === defs.constants.NO_ROUTE) {
        err = new Error('message unroutable');
        self._returnMessage = null;
      }
      cb(err);
    }
  }));
