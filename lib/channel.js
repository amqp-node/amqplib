...
C._rpc = function(method, fields, expect) {
  ...
  return reply.promise.then(function(f) {
    if (f.id === expect)
      ...
      var err = fmt("Expected %s; got %s",
                    methodName(expect), inspect(f, false));
...
C.stop = function() {
  ...
  // pending may have already been cleared in the case where we close 
  // the connection and then the channel
  if (this.pending) {
    var discard;
