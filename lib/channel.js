...
Channel.closedEnqueue = function(_method, _fields, reply) {
  reply(new Error("Channel is closed"));
};
...
C.stop = function() {
  ...
  // to fail; but they mustn't respond by trying to sending more.
  this.emit('end');
...
C.closeWithError = function(reason, code, error, cb) {
  var toClosed = this.toClosed.bind(this, error);
