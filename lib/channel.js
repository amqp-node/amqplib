...
C.toClosed = function() {
  var stack = stackCapture();
  this._rejectPending();
  invalidateSend(this, 'Channel closed', stack);
  this.accept = invalidOp('Channel closed', stack);
  this.connection.releaseChannel(this.ch);
  this.emit('close');
};

// Stop being able to send and receive methods and content. Used when
// we close the channel. Calls the callback once the server has
// acknowledged the close, but before the channel is moved to the
// closed state.
C.toClosing = function(cb) {
  ...
  var stack = stackCapture();
  invalidateSend(this, 'Channel closing', stack);
};
...
C.accept = function(f) {
  ...
  case defs.ChannelClose:
    ...
    var e = new Error("Channel closed by server: " + closeMsg(f));
