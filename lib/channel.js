...
function Channel(connection, options) {
  ...
  this.on(
    "ack",
    this.handleConfirm.bind(this, function (cb) {
      if (cb) cb(null);
    }),
  );
  this.on(
...
C.sendOrEnqueue = function (method, fields, reply) {
  if (!this.reply) {
    ...
    this.pending.push({ method: method, fields: fields, reply: reply });
  }
};
...
// low-level machinery.
function BaseChannel(connection, options) {
  Channel.call(this, connection, options);
  this.consumers = {};
}
