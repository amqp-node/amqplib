...
C.open = function(credentials, openCallback0) {
  ...
  var allFields = Object.create(tunedOptions);
  var k, i = -1;
  var keys = Object.keys(credentials);
  while((k = keys[++i])) {
    allFields[k] = credentials[k];
  }
...
C.sendMessage = function(channel, Method, fields, Properties, props, content) {
  ...
  if(!(content instanceof Blob)) {
    throw new TypeError('content is not a Blob');
  }
  var reader = new FileReader();
  reader.onloadend = function() {
    content = Buffer.from(new Uint8Array(reader.result));
    var mframe = encodeMethod(Method, channel, fields);
    var pframe = encodeProperties(Properties, channel, content.length, props);
    var webSocket = self.channels[channel].webSocket;
    self.sentSinceLastCheck = true;
    ...
    if(allLen < SINGLE_CHUNK_THRESHOLD) {
      ...
      return self.sendContent(channel, content);
    }
  };
