...
function reconnect(connection, callback) {
  connect(connection.url, connection.socketOptions, callback, connection);
}
...
function connect(url, socketOptions, openCallback, connection) {
  ...
  function onConnect() {
    ...
    if(connection == undefined){
      ...
      c = connection.changeSocket(sock);
    }
    c.url = url;
    ...
    c.recoverAfter = sockopts.recoverAfter || 0;
    // Time to wait after reconnect fails
    c.recoverTimeout = sockopts.recoverTimeout || 0;
    // N times to retry reconnecting
    c.recoverRetries = sockopts.recoverRetries || 0;
