...
function reconnect(connection, callback) {
  connect(connection.url, connection.socketOptions, function(err, reconnectedConnection){
    if(err == null){
      reconnectedConnection.recover(callback);
    } else {
      callback(err, reconnectedConnection);
    }
  }, connection);
}

function connect(url, socketOptions, openCallback, connection) {
  ...
  function onConnect() {
    ...
    if(connection == undefined){
      ...
      c = connection.reconnect(sock);
    }
    c.url = url;
    c.socketOptions = socketOptions;
    c.reconnect_enabled = sockopts.reconnect_enabled;
